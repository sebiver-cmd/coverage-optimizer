...
_SKU_NAMES = [
    'sku', 'article', 'artikelnr', 'varenr', 'item', 'item number',
    'part', 'artikelnummer', 'product number', 'productnumber',
    'model', 'mpn', 'reference', 'ref', 'nummer', 'number',
    'art.nr', 'artnr', 'bestillingsnr', 'ordernumber',
    'artikelcode', 'itemcode', 'code', 'produktnummer', 'art. nr.',
]
_PRICE_NAMES = [
...
def optimize_prices(
    ...
    """Apply optional cost-price adjustment, calculate coverage and optimise.

    Parameters
    ----------
    cost_pct : float
        Percentage adjustment to apply to all BUY_PRICE (cost price)
        values before recalculating coverage (e.g. 10.0 = +10 %).
        Simulates a supplier price increase/decrease.
    ...
    # Detect manual BUY_PRICE edits BEFORE applying the global cost adjustment
    ...
    # Apply optional cost-price (BUY_PRICE) adjustment
    if cost_pct != 0:
        df['BUY_PRICE_NUM'] = df['BUY_PRICE_NUM'] * (1 + cost_pct / 100)

    # Reformat BUY_PRICE from (possibly adjusted) numeric values
...
with st.sidebar:
    ...
    cost_pct = st.number_input(
        ...
        step=0.5,
        help=(
            "Simulate a supplier price change. Increase or decrease all "
            "cost prices (BUY_PRICE) by this percentage to see how it "
            "affects coverage rates. For example, enter 10 to simulate "
            "a 10 % cost increase across all products."
        ),
    )
...
if uploaded_file is not None:
    try:
        ...
        final_df, adjusted_count, adjusted_mask, import_df = optimize_prices(
            work_df, cost_pct, original_buy_prices=parsed_df['BUY_PRICE_NUM'],
        )
        ...
        if total:
            ...
            mcol5.metric(
                "Avg Coverage",
                f"{adj_avg:.1f}%",
                delta=f"{delta:+.1f}%" if cost_pct != 0 else None,
            )
        st.markdown("")
